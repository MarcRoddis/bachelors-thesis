---
title: "Simulation of imputation of censored values for June 30th Zoom Call v5"
author: "Marc Roddis"
date: "6/29/2020"
...
```{r chunk5b1}
data_tib2 = as_tibble(get_sim_data() )
...
for (iter in 1:n_iter) {
  sim_data_best <- get_sim_data()
  data_tib2 = as_tibble(sim_data_best)
  data_geomean2 <- data_tib2 %>%
    group_by(year) %>%
    summarise_at(vars(cb28, cb153), geomean_prelogged  ) %>%
  ungroup()
...
for (iter in 1:n_iter) {
  sim_omit_data <- get_sim_data()
  data_omit_tib <- as_tibble(sim_omit_data)
  cb28_cprop <- quantile(data_omit_tib$cb28,probs = cprop, names=FALSE)  
...
for (iter in 1:n_iter) {
  sim_subst_data <- get_sim_data()
  data_subst_tib <- as_tibble(sim_subst_data)
  cb28_cprop <- quantile(data_subst_tib$cb28,probs = cprop, names=FALSE)
...
for (iter in 1:n_iter) {
  dat <- get_sim_data()
  dat_cenreg_tib = as_tibble(dat)
  cb28_cprop <- quantile(dat_cenreg_tib$cb28,probs = cprop, names=FALSE)
  data_indic <- dat_cenreg_tib %>%     
...
for (iter in 1:n_iter) {
  ...
  cb28_cprop <- quantile(dat_tib$cb28,probs = cprop, names=FALSE)  
  data_cenS <- dat_tib %>%     
    mutate(cb28 = pmax(cb28, cb28_cprop ) ) 
...
mse_ros_beta = mean( (estimates_ros_beta - true_beta28year)^2 )
sq_bias_ros_beta = (mean(estimates_ros_beta) - true_beta28year)^2
var_ros_beta = var(estimates_ros_beta)
mse_ros_beta
sq_bias_ros_beta
var_ros_beta

mse_censReg_beta = mean( (estimates_censReg_beta - true_beta28year)^2 )
sq_bias_censReg_beta = (mean(estimates_censReg_beta) - true_beta28year)^2
var_censReg_beta = var(estimates_censReg_beta)
mse_censReg_beta
sq_bias_censReg_beta
var_censReg_beta

mse_cenreg_beta = mean( (estimates_cenreg_beta - true_beta28year)^2 )
sq_bias_cenreg_beta = (mean(estimates_cenreg_beta) - true_beta28year)^2
var_cenreg_beta = var(estimates_cenreg_beta)
mse_cenreg_beta
sq_bias_cenreg_beta
var_cenreg_beta

mse_subst_beta = mean( (estimates_subst_beta - true_beta28year)^2 )
sq_bias_subst_beta = (mean(estimates_subst_beta) - true_beta28year)^2
var_subst_beta = var(estimates_subst_beta)
mse_subst_beta
sq_bias_subst_beta
var_subst_beta

mse_omit_beta = mean( (estimates_omit_beta - true_beta28year)^2 )
sq_bias_omit_beta = (mean(estimates_omit_beta) - true_beta28year)^2
var_omit_beta = var(estimates_omit_beta)
mse_omit_beta
sq_bias_omit_beta
var_omit_beta
...
for (iter in 1:n_iter) {
dat <- get_sim_data(sample_size=10)
dat_tib = as_tibble(dat)
cb28_p20 <- quantile(dat_tib$cb28, probs = 0.2)
data_indic <- dat_tib %>%     
  mutate(cb28 = pmax(cb28, cb28_p20 )) %>%
  mutate(ci28 = cb28 == cb28_p20 ) 
data_indic_ros_aug <- data_indic %>%
  group_by(year) %>%
  mutate(mean_ros = mean(ros(cb28, ci28, forwardT=NULL, reverseT=NULL) ) ) %>%
  mutate(sd_ros = sd(ros(cb28, ci28, forwardT=NULL, reverseT=NULL) ) ) %>%
  ungroup()
ros_trial_lm <- lm(data_indic_ros_aug$mean_ros ~ data_indic_ros_aug$year)
estimates_ros_beta[iter] <- coef(ros_trial_lm)[2]
}
...
plot_all_variance
coef(censReg_fit1)
glimpse(data_indic_df$cb28)
glimpse(data_indic_df$ci28)

# ros_trial1 <- ros(data_indic_df$cb28, data_indic_df$ci28, forwardT=NULL, reverseT=NULL)
# summary(ros_trial1)
# mean(ros_trial1)
# sd(ros_trial1)
# cenros_trial_df <- exp(data_indic_df$cb28)
# cenros_trial1 <- cenros(cenros_trial_df, data_indic_df$ci28) #From NADA book
# cenros_trial1 #Does not work
# ros_predict1 <- predict(ros_trial1, data_indic_df$cb28)
# ros_predict1
```
