---
title: "Analysis of censored PCB data using regression imputation (main report) v7"
author: "Marc Roddis"
date: "6/20/2020"
...
We begin with `pcb_tib4` which was created as described in my document "Cleaning the pcb dataset".  We will first mutate `pcb_tib4` (and denote this mutated tibble as `pcb_tib4m`)  so that it contains the logarithms of the all pcb concentrations instead of the concentrations themselves.  In this section, we will explore the effect of five variables (location `LOC`, species `SPECIES`, age `ALDR`, year `YEAR`, percentage fat `FPRC`) on the association between x=CB138 and y=CB153.  For each variable, we will look for evidence of confounding from: 

1. The appearance of the scatter plot.

2. The adjusted R-squared value and the slope coefficient `cb138_beta_cb153` for the fitted linear model.  
...
To remove potential confounding by species, we will use observations from herring exclusively for all our subsequent analyses.   The dataset `pcb_tib4h` (3269 observations) was obtained from the original dataset `pcb.csv` (5056 observations) by removing:  28 observations that have NA values for all variables except CB138; all 11 observations for which the CB138 value was censored; all observations except those for herring.

The resulting `pcb_tib4h` dataset has (471, 295, 39, 16, 0, 0, 30) NA values and (1163, 408, 9, 3, 0, 0, 310) censored values for (CB28, CB52, CB101, CB118, CB138, CB153, CB180) respectively.  So for CB28 and CB52 the number of NA values and censored values were both highest and second highest respectively, whereas there were no such values for both CB138 and CB153.  For this reason, we created the smaller dataset `pcb_tib4hs` from `pcb_tib4h` by removing the variables CB101, CB118, CB180.

There are only 94 observations prior to the year 1989 whereas there are 3175 observations between 1989 and 2017.  For this reason, we created the smaller dataset `pcb_tib4hsy` from `pcb_tib4hs` by removing the observations from years prior to 1989, and setting 1989 as "year zero". 

We will use `pcb_tib4hsy` as our starting point for the analyses reported in the next section. 

```{r chunk107b, include=FALSE}
pcb_tib4h <- pcb_tib4 %>%
  filter(SPECIES == "Herring")
# sum(is.na(pcb_tib4h$CB28)) was used to find number of M_i for all CBn
# v28 <- na.omit(pcb_tib4h$CB28)
# sum(v28<0) was used to find number of C_i for all CBn
pcb_tib4hs <- pcb_tib4h %>%
  select(-(CB101),-(CB118),-(CB180))
# sum(pcb_tib4hs$YEAR == 1987) To find number of observations by YEAR 
pcb_tib4hsy <- pcb_tib4hs %>%
  filter(YEAR >= 1989) %>%
...
We begin with dataset `pcb_tib4hsy`,   So we will use the complete data for CB138 and CB153 and the strong correlations between pcb concentrations to impute values for the observations with censored and/or missing values for CB28 and/or CB52.   

We will perform simulations studies in which we investigate the effect of the three variables `sd`, `beta` and `LOD`:  `sd` represents the standard deviation of the pcb concentrations that are being imputed (CB28 or CB52), `beta` represents the slope of the linear regression line when $x=YEAR$ and either $y=CB28$ or $y=CB52$, `LOD` represents the level of detection, which is the threshold value used to determine whether an observation gets censored or not. 
...
We will use fixed values for the parameters `sd_cb` and `beta_cb`:  `sd_cb` represents the standard deviation of CB153 which we hold fixed at the value calculated from the dataset because the data for CB153 is complete, `beta_cb` represents the slope of the linear regression line when x=CB153 and either y=CB28 or y=CB52 .  
...
Therefore despite its stated drawbacks, we begin by fitting linear models by omission of missing values.  We obtain the values (-0.054, -0.061, -0.061, -0.046) for (beta_cb28, beta_cb52, beta_cb138, beta_cb153) respectively, and (0.41, 0.33, 0.29, 0.19) for the corresponding adjusted R-squared values.   We will later compare the parameters obtained from the simple approach with those obtained by imputation.  Note that we are using beta_cb153 to guard against erroneous results since we know that the values obtained here and by MI later must be equal because vB1h$CB153 contains no missing values.  The adjusted R-squared value for the fitted linear model for $x=YEAR$, $y=CB28$ was 0.41.

A linear model was also fitted for $x=CB153$, $y=CB28$; the slope cb28_beta_cb153 of the regression line was 0.56 and the adjusted R-squared was 0.54.

```{r chunk201, include=FALSE}
simple_fit_CB28_CB153 <- lm(CB28 ~ CB153, data=vB1hsy, na.action = na.omit)
simple_fit_CB52_CB153 <- lm(CB52 ~ CB153, data=vB1hsy, na.action = na.omit)
simple_fit_CB28_YEAR <- lm(CB28 ~ YEAR, data=vB1hsy, na.action = na.omit)
simple_fit_CB52_YEAR <- lm(CB52 ~ YEAR, data=vB1hsy, na.action = na.omit)
simple_fit_CB138_YEAR <- lm(CB138 ~ YEAR, data=vB1hsy, na.action = na.omit)
simple_fit_CB153_YEAR <- lm(CB153 ~ YEAR, data=vB1hsy, na.action = na.omit)
```

```{r chunk201b}
summary(simple_fit_CB28_CB153)
summary(simple_fit_CB52_CB153)
summary(simple_fit_CB28_YEAR)
summary(simple_fit_CB52_YEAR)
summary(simple_fit_CB138_YEAR)
...
pred <- quickpred(vB1hsy, minpuc = 0.5)
qpred1 <- mice(vB1hsy, pred = pred, seed = 29725)
completed_vB1hsy <- complete(qpred1)
```

```{r chunk203}
par(mfrow=c(1,2))
plotMI1 <- plot(vB1hsy$CB28 ~ vB1hsy$CB153)
plotMI2 <- plot(completed_vB1hsy$CB28 ~ completed_vB1hsy$CB153)
plotMI3 <- plot(vB1hsy$CB28 ~ vB1hsy$YEAR)
plotMI4 <- plot(completed_vB1hsy$CB28 ~ completed_vB1hsy$YEAR)

# plot_grid(plotMI1, plotMI2, labels = "AUTO")
```

```{r chunk203a, include=FALSE}
completed_fit_28_153 <- lm(CB28 ~ CB153, data=completed_vB1hsy)
completed_fit_52_153 <- lm(CB52 ~ CB153, data=completed_vB1hsy)
completed_fit_28_YEAR <- lm(CB28 ~ YEAR, data=completed_vB1hsy)
completed_fit_52_YEAR <- lm(CB52 ~ YEAR, data=completed_vB1hsy)
```

```{r chunk203b}
summary(completed_fit_28_153)
summary(completed_fit_52_153)
summary(completed_fit_28_YEAR)
summary(completed_fit_52_YEAR)
# below code allows check (since CB138 and CB153 are complete)
# completed_fit_138_YEAR <- lm(CB138 ~ YEAR, data=completed_vB1hsy) 
# completed_fit_153_YEAR <- lm(CB153 ~ YEAR, data=completed_vB1hsy)
# summary(completed_fit_138_YEAR)
# summary(completed_fit_153_YEAR)
```

```{r chunk203c}
# Exploratory modeling using multiple linear regression.  
# Outside scope of our report

# completed_fit1_CB28_all_sig_predictors <- lm(CB28 ~ YEAR + LAT + LONG + ALDR + CB52 + CB138 + CB153, data=completed_vB1hsy)
# completed_fit1_CB52_all_sig_predictors <- lm(CB52 ~ YEAR + LONG + FPRC + CB28 + CB138 + CB153, data=completed_vB1hsy)
# completed_fit1_CB138_all_sig_predictors <- lm(CB138 ~ YEAR + LAT + ALDR + CB52 + CB28 + CB153, data=completed_vB1hsy)
# completed_fit1_CB153_all_sig_predictors <- lm(CB153 ~ YEAR + LAT + ALDR + FPRC + CB52 + CB138 + CB28, data=completed_vB1hsy)
# summary(completed_fit_28_153)
...
# pred[c(12:15),] # Table showing predictors of PCB concentrations
```
...
##### Estimation of beta_cb and sd_cb  from linear models fitted to dataset `completed_vB1hs`

We used MI to replace NA values of CB28 with imputed values which allowed us to calculate the standard deviation of (the logarithm of) CB28 from the completed dataset.  We obtained the values (-0.052, -0.047, 0.71, 0.93) for (beta_cb28, beta_cb153, sd_cb28, sd_cb153) respectively; these values are used to choose baseline parameter value for our simulation studies as described below.   We also see that the value beta_cb28=-0.052 we obtained by MI is very similar to the value beta_cb28=-0.054 that we obtained initially by simple omission of missing values.  We did indeed get identical values for beta_cb153 by both methods, so our MI analysis passed this simple quality control.  

```{r chunk204}
# This code seems duplicated for some reason.
# sd(vB1hsy$CB28) # returns NA because CB28 has NA values
# sd(vB1hsy$CB153)
# sd(completed_vB1hsy$CB28) # does not return NA
# sd(completed_vB1hsy$CB153) # same as sd(vB1$CB153) because there were no NAs
# completed_fit_28_YEAR <- lm(CB28 ~ YEAR, data=completed_vB1hsy)
# completed_fit_153_YEAR <- lm(CB153 ~ YEAR, data=completed_vB1hsy)
# completed_fit_28_153 <- lm(CB28 ~ CB153, data=completed_vB1hsy)
# summary(completed_fit_28_YEAR)
...
We show below a scatter plot and fitted linear model summary for CB28 versus YEAR from `CB28_filtered4mhs`; this dataset was created from `pcb_tib4mhs` by removal of all censored observations $C_i$.  The fitted linear model for $x=YEAR$, $y=CB28$ has na.omit as the na.action by default; for this model, the slope coefficient is -0.045; SE = 0.0018;  p-value < 2e-16; Adjusted R-squared = 0.29.

```{r chunk205, include=FALSE}
CB28_filtered4hsy <- pcb_tib4hsy %>%
  select(CB28, CB153, YEAR) %>%
  mutate(CB28 = ifelse(CB28> -0.0001 & CB28< 0.0001, NA, CB28) ) %>%
  # the line above re-codes 360 zero values as NA 
  filter(CB28 > 0) %>%
  mutate(CB28 = round(log(CB28),4) )
CB52_filtered4hsy <- pcb_tib4hsy %>%
  select(CB52, CB153, YEAR) %>%
  mutate(CB52 = ifelse(CB52> -0.0001 & CB52< 0.0001, NA, CB52) ) %>%
  # the line above re-codes 360 zero values as NA 
  filter(CB52 > 0) %>%
  mutate(CB52 = round(log(CB52),4) )
...
# plot(CB28_filtered4hsy$CB28 ~ CB28_filtered4hsy$YEAR)
# plot(CB52_filtered4hsy$CB28 ~ CB52_filtered4hsy$YEAR)
CB28_year_filtered <- lm(CB28_filtered4hsy$CB28 ~ CB28_filtered4hsy$YEAR)
CB28_CB153_filtered <- lm(CB28_filtered4hsy$CB28 ~ CB28_filtered4hsy$CB153)
CB52_year_filtered <- lm(CB52_filtered4hsy$CB52 ~ CB52_filtered4hsy$YEAR)
CB52_CB153_filtered <- lm(CB52_filtered4hsy$CB52 ~ CB52_filtered4hsy$YEAR)
```


```{r chunk206b}
summary(CB28_year_filtered)
summary(CB28_CB153_filtered)
summary(CB52_year_filtered)
summary(CB52_CB153_filtered)
```
...
In this approach we used the cenreg() method to fit a linear model for dataset `pcb_tib4hs` without removing or substituting censored values, instead the information contained in the censored values is used to obtain the fitted model `tib4hs_cenreg_28` for $x=YEAR$, $y=CB28$, which has coefficients (0.010255, -0.000353) for (intercept, beta_CB28).   

```{r chunk207}
pcb_tib4hsy_CB28CI <- pcb_tib4hsy %>%
  filter( !( is.na(CB28) ) ) %>%
  mutate(CI28 = CB28 < 0) %>%
  mutate(CB28 = ifelse(CB28< 0, abs(CB28), CB28) )
pcb_tib4hsy_CB52CI <- pcb_tib4hsy %>%
  filter( !( is.na(CB52) ) ) %>%
  mutate(CI52 = CB52 < 0) %>%
  mutate(CB52 = ifelse(CB52< 0, abs(CB52), CB52) )
library("NADA")
tib4hsy_cenreg_28 = cenreg(Cen(pcb_tib4hsy_CB28CI$CB28, pcb_tib4hsy_CB28CI$CI28)~ pcb_tib4hsy_CB28CI$YEAR, dist="gaussian")
tib4hsy_cenreg_28
tib4hsy_cenreg_52 = cenreg(Cen(pcb_tib4hsy_CB52CI$CB52, pcb_tib4hsy_CB52CI$CI52)~ pcb_tib4hsy_CB52CI$YEAR, dist="gaussian")
tib4hsy_cenreg_52
```

```{r chunk207b}
plot(pcb_tib4hsy_CB28CI$CB28 ~ pcb_tib4hsy_CB28CI$YEAR)
plot(pcb_tib4hsy_CB28CI$CB28 ~ pcb_tib4hsy_CB28CI$CB153)
plot(pcb_tib4hsy_CB52CI$CB52 ~ pcb_tib4hsy_CB52CI$YEAR)
plot(pcb_tib4hsy_CB52CI$CB52 ~ pcb_tib4hsy_CB52CI$CB153)
```

We will now create model `tib4hsy_CB28LMpredSUB1` by substituting the CB28 censored values with values predicted by the regression equation of the model `tib4hs_cenreg_28` and fitting a linear model to the resulting dataset.  This model gives adjusted R-squared = 0.56, beta_CB28 = -0.075413.  This adjusted R-squared value is slightly higher than we got previously (without using cenreg() ).  However, this is as we expected because substitution using predicted values has given plotted points that lie on the regression line so the increase in R-squared is at the expense of unrealistic alignment of the substituted points.   See our document "Preliminary studies of censored data" to see how such alignment can be eliminated by the addition of noise. 

```{r chunk208}
pcb_tib4hsy_CB28LMpredSUB1 <- pcb_tib4hsy %>%
  filter( !( is.na(CB28) ) ) %>%
  mutate(CB28 = ifelse(CB28<= 0, 0.010255 -0.000353*YEAR, CB28 ) ) %>%
  mutate(CB28 = log(CB28))
tib4hsy_CB28_year_LMpredSUB1 <- lm(pcb_tib4hsy_CB28LMpredSUB1$CB28 ~ pcb_tib4hsy_CB28LMpredSUB1$YEAR)
tib4hsy_CB28_CB153_LMpredSUB1 <- lm(pcb_tib4hsy_CB28LMpredSUB1$CB28 ~ pcb_tib4hsy_CB28LMpredSUB1$CB153)
# plot(tib4hsy_CB28_year_LMpredSUB1$CB28 ~ tib4hsy_CB28_year_LMpredSUB1$YEAR)
# plot(tib4hsy_CB28_CB153_LMpredSUB1$CB28 ~ tib4hsy_CB28_CB153_LMpredSUB1$YEAR)
summary(tib4hsy_CB28_year_LMpredSUB1)
summary(tib4hsy_CB28_CB153_LMpredSUB1)
```

```{r chunk208b}
pcb_tib4hsy_CB52LMpredSUB1 <- pcb_tib4hsy %>%
  filter( !( is.na(CB52) ) ) %>%
  mutate(CB52 = ifelse(CB52<= 0, 0.010255 -0.000353*YEAR, CB52 ) ) %>%
  mutate(CB52 = log(CB52))
tib4hsy_CB52_year_LMpredSUB1 <- lm(pcb_tib4hsy_CB52LMpredSUB1$CB52 ~ pcb_tib4hsy_CB52LMpredSUB1$YEAR)
tib4hsy_CB52_CB153_LMpredSUB1 <- lm(pcb_tib4hsy_CB52LMpredSUB1$CB52 ~ pcb_tib4hsy_CB52LMpredSUB1$CB153)
# plot(tib4hsy_CB52_year_LMpredSUB1$CB28 ~ tib4hsy_CB52_year_LMpredSUB1$YEAR)
# plot(tib4hsy_CB52_CB153_LMpredSUB1$CB28 ~ tib4hsy_CB52_CB153_LMpredSUB1$YEAR)
summary(tib4hsy_CB52_year_LMpredSUB1)
summary(tib4hsy_CB52_CB153_LMpredSUB1)
```

We will now exclude both the observations for which the value of CB28 is missing and those for which it is censored and fit a linear model and compare with our previous results.  The resulting fitted model has coefficients (-4.650412 , -0.042518) and Adjusted-R-squared = 0.26.

```{r chunk209}
# noise <- rnorm(sample-size)
pcb_tib4hsy_CB28lmEXCLUDE <- pcb_tib4hsy %>%
  filter( !( is.na(CB28) ) ) %>%
  filter( CB28 > 0  ) %>%
  mutate(CB28 = log(CB28))
tib4hsy_CB28lmEXCLUDE <- lm(pcb_tib4hsy_CB28lmEXCLUDE$CB28 ~ pcb_tib4hsy_CB28lmEXCLUDE$YEAR)
summary(tib4hsy_CB28lmEXCLUDE)
plot(pcb_tib4hsy_CB28lmEXCLUDE$CB28 ~ pcb_tib4hsy_CB28lmEXCLUDE$YEAR)
```

```{r chunk210}
# pcb_tib4hsy_CB28lmEXCLUDE_predSUB <- pcb_tib4hsy %>%
#   filter( !( is.na(CB28) ) ) %>%
#   mutate(CB28 = ifelse(CB28<= 0, -4.650412 -0.042518*YEAR, CB28 ) ) %>%
#   mutate(CB28 = log(CB28))
# tib4hsy_CB28lmEXCLUDE_predSUB <- lm(pcb_tib4hsy_CB28lmEXCLUDE_predSUB$CB28 ~ pcb_tib4hsy_CB28lmEXCLUDE_predSUB$YEAR)
# summary(tib4hsy_CB28lmEXCLUDE_predSUB)
# plot(pcb_tib4hsy_CB28lmEXCLUDE_predSUB$CB28 ~ pcb_tib4hsy_CB28lmEXCLUDE_predSUB$YEAR)
```
...
tib4hsy_year_sqrt2_sub <- pcb_tib4hsy %>%
  # filter(YEAR >= 1987) %>%
  mutate_at(vars(CB28,CB52,CB138,CB153), fix_NA) %>%
  mutate_at(vars(CB28,CB52,CB138,CB153), fix_loq) %>%
  # mutate_at(vars(YEAR), zero_at_1987) %>%
  group_by(YEAR) %>% 
  summarise_at(vars(CB28,CB52,CB138,CB153), geomean  ) %>% 
  ungroup()
```
...
```{r chunk212}
par(mfrow=c(2,2))
plot(tib4hsy_year_sqrt2_sub$CB28 ~ tib4hsy_year_sqrt2_sub$YEAR)
plot(tib4hsy_year_sqrt2_sub$CB52 ~ tib4hsy_year_sqrt2_sub$YEAR)
plot(tib4hsy_year_sqrt2_sub$CB28 ~ tib4hsy_year_sqrt2_sub$CB153)
plot(tib4hsy_year_sqrt2_sub$CB52 ~ tib4hsy_year_sqrt2_sub$CB153)
```

```{r chunk213, include=FALSE}
fitCB28_year_sqrt2_sub <- lm(tib4hsy_year_sqrt2_sub$CB28 ~ tib4hsy_year_sqrt2_sub$YEAR)
fitCB52_year_sqrt2_sub <- lm(tib4hsy_year_sqrt2_sub$CB52 ~ tib4hsy_year_sqrt2_sub$YEAR)
fitCB28_CB153_sqrt2_sub <- lm(tib4hsy_year_sqrt2_sub$CB28 ~ tib4hsy_year_sqrt2_sub$CB153)
fitCB52_CB153_sqrt2_sub <- lm(tib4hsy_year_sqrt2_sub$CB52 ~ tib4hsy_year_sqrt2_sub$CB153)
```

Summary output for fitted models for x=YEAR or x=CB153, and y=CB28 or y=CB52 for this approach are shown below.

```{r chunk214}
summary(fitCB28_year_sqrt2_sub)
summary(fitCB52_year_sqrt2_sub)
summary(fitCB28_CB153_sqrt2_sub)
summary(fitCB52_CB153_sqrt2_sub)
```
...
The `tib4h_year_remove_cens` dataset was created from our starting dataset `pcb_tib4h` by removing observations from before 1987, setting 1987 as the zero year, removing observations with censored values, substituting log-concentrations with annual (geometric) mean log-concentrations.  The four scatter plots for x=YEAR or x=CB153, and y=CB28 or y=CB52 from this created dataset are shown below.
...
tib4hsy_year_remove_cens <- pcb_tib4hsy %>%
  # filter(YEAR >= 1987) %>%
  mutate_at(vars(CB28,CB52,CB138,CB153), fix_NA) %>%
  mutate_at(vars(CB28,CB52,CB138,CB153), remove_cens) %>%
  # mutate_at(vars(YEAR), zero_at_1987) %>%
  group_by(YEAR) %>% 
  summarise_at(vars(CB28,CB52,CB138,CB153), geomean  ) %>% 
  ungroup()
...
```{r chunk216}
par(mfrow=c(2,2))
plot(tib4hsy_year_remove_cens$CB28 ~ tib4hsy_year_remove_cens$YEAR)
plot(tib4hsy_year_remove_cens$CB52 ~ tib4hsy_year_remove_cens$YEAR)
plot(tib4hsy_year_remove_cens$CB28 ~ tib4hsy_year_remove_cens$CB153)
plot(tib4hsy_year_remove_cens$CB52 ~ tib4hsy_year_remove_cens$CB153)
```

```{r chunk217, include=FALSE}
fitCB28_year_remove_cens <- lm(tib4hsy_year_remove_cens$CB28 ~ tib4hsy_year_remove_cens$YEAR)
fitCB52_year_remove_cens <- lm(tib4hsy_year_remove_cens$CB52 ~ tib4hsy_year_remove_cens$YEAR)
fitCB28_CB153_remove_cens <- lm(tib4hsy_year_remove_cens$CB28 ~ tib4hsy_year_remove_cens$CB153)
fitCB52_CB153_remove_cens <- lm(tib4hsy_year_remove_cens$CB52 ~ tib4hsy_year_remove_cens$CB153)
# fitCB138_year_remove_cens <- lm(tib4hsy_year_remove_cens$CB138 ~ tib4hsy_year_remove_cens$YEAR)
# fitCB153_year_remove_cens <- lm(tib4hsy_year_remove_cens$CB153 ~ tib4hsy_year_remove_cens$YEAR)
```

Linear models were fitted for $x=YEAR$, $y=CBn$ where n denotes 28, 52, 138, 153 respectively.  The values of (beta_cb28, beta_cb52, beta_cb138, beta_cb153) given by the model are (-3.9e-04, -7.5e-04, -0.0040, -0.0037), and the adjusted R-squared values are (0.57, 0.67, 0.71, 0.69).  As before, we see that the adjusted R-squared values are higher for the averaged data than the corresponding values (0.41, 0.33, 0.29, 0.19) we reported previously from applying the same methodology to the corresponding unaveraged values. 

A linear model was also fitted for $x=CB153$, $y=CB28$; the slope cb28_beta_cb153 of the regression line was 0.073 and the adjusted R-squared was 0.36.
...
summary(fitCB28_CB153_remove_cens)
summary(fitCB52_CB153_remove_cens)
...
The adjusted R-squared values for CB28:CB180 for the fitted model after substitution using LOQ/sqrt(2) were (0.55, 0.64, 0.65, 0.65, 0.72, 0.75, 0.72).  The corresponding values after exclusion of censored data were (0.53, 0.64, 0.68, 0.64, 0.72, 0.75, 0.71).  We see that the values in these two vectors are very similar but much higher than the corresponding values for non-averaged data (see our previous document).

#### Using LOQ/sqrt(2) substituted censored values for data from 1989 onwards

```{r chunk220, include=FALSE}
# zero_at_1989 <- function(x) x-1989 # Set 1989 as year zero.
tib4hsy_year_sqrt2_sub_start1989 <- pcb_tib4hsy %>%
  # filter(YEAR >= 1989) %>%
  mutate_at(vars(CB28,CB52,CB138,CB153), fix_NA) %>%
  mutate_at(vars(CB28,CB52,CB138,CB153), fix_loq) %>%
  # mutate_at(vars(YEAR), zero_at_1989) %>%
  group_by(YEAR) %>% 
  summarise_at(vars(CB28,CB52,CB138,CB153), geomean  ) %>% 
  ungroup()
```
...
```{r chunk221}
par(mfrow=c(2,2))
plot(tib4hsy_year_sqrt2_sub_start1989$CB28 ~ tib4hsy_year_sqrt2_sub_start1989$YEAR)
plot(tib4hsy_year_sqrt2_sub_start1989$CB52 ~ tib4hsy_year_sqrt2_sub_start1989$YEAR)
plot(tib4hsy_year_sqrt2_sub_start1989$CB28 ~ tib4hsy_year_sqrt2_sub_start1989$CB153)
plot(tib4hsy_year_sqrt2_sub_start1989$CB52 ~ tib4hsy_year_sqrt2_sub_start1989$CB153)
```

```{r chunk222, include=FALSE}
fitCB28_year_sqrt2_sub_from1989 <- lm(tib4hsy_year_sqrt2_sub_start1989$CB28 ~ tib4hsy_year_sqrt2_sub_start1989$YEAR)
fitCB52_year_sqrt2_sub_from1989 <- lm(tib4hsy_year_sqrt2_sub_start1989$CB52 ~ tib4hsy_year_sqrt2_sub_start1989$YEAR)
fitCB28_CB153_sqrt2_sub_from1989 <- lm(tib4hsy_year_sqrt2_sub_start1989$CB28 ~ tib4hsy_year_sqrt2_sub_start1989$CB153)
fitCB52_CB153_sqrt2_sub_from1989 <- lm(tib4hsy_year_sqrt2_sub_start1989$CB52 ~ tib4hsy_year_sqrt2_sub_start1989$CB153)
```

```{r chunk223}
summary(fitCB28_year_sqrt2_sub_from1989)
summary(fitCB52_year_sqrt2_sub_from1989)
summary(fitCB28_CB153_sqrt2_sub_from1989)
summary(fitCB28_CB153_sqrt2_sub_from1989)
```

#### Using exclusion of censored values for data from 1989 onwards

```{r chunk224, include=FALSE}
remove_cens <- function(x) ifelse(x < 0, NA, x) # Substitute LOQ by NA, which then gets removed by our geomean function
tib4hsy_year_remove_cens_from1989 <- pcb_tib4hsy %>%
  # filter(YEAR >= 1989) %>%
  mutate_at(vars(CB28,CB52,CB138,CB153), fix_NA) %>%
  mutate_at(vars(CB28,CB52,CB138,CB153), remove_cens) %>%
  # mutate_at(vars(YEAR), zero_at_1989) %>%
...
```{r chunk225}
par(mfrow=c(2,2))
plot(tib4hsy_year_remove_cens_from1989$CB28 ~ tib4hsy_year_remove_cens_from1989$YEAR)
plot(tib4hsy_year_remove_cens_from1989$CB52 ~ tib4hsy_year_remove_cens_from1989$YEAR)
plot(tib4hsy_year_remove_cens_from1989$CB28 ~ tib4hsy_year_remove_cens_from1989$CB153)
plot(tib4hsy_year_remove_cens_from1989$CB28 ~ tib4hsy_year_remove_cens_from1989$CB153)
```

```{r chunk226, include=FALSE}
fitCB28_year_remove_cens_from1989 <- lm(tib4hsy_year_remove_cens_from1989$CB28 ~ tib4hsy_year_remove_cens_from1989$YEAR)
fitCB52_year_remove_cens_from1989 <- lm(tib4hsy_year_remove_cens_from1989$CB52 ~ tib4hsy_year_remove_cens_from1989$YEAR)
fitCB28_CB153_remove_cens_from1989 <- lm(tib4hsy_year_remove_cens_from1989$CB28 ~ tib4hsy_year_remove_cens_from1989$CB153)
fitCB52_CB153_remove_cens_from1989 <- lm(tib4hsy_year_remove_cens_from1989$CB28 ~ tib4hsy_year_remove_cens_from1989$CB153)
```

```{r chunk227}
summary(fitCB28_year_remove_cens_from1989)
summary(fitCB52_year_remove_cens_from1989)
summary(fitCB28_CB153_remove_cens_from1989)
summary(fitCB52_CB153_remove_cens_from1989)
```

#### Intermediate (second) summary

As stated previously: starting from 1987 the adjusted R-squared values for CB28:CB180 for the fitted model after substitution using LOQ/sqrt(2) were (0.55, 0.64, 0.65, 0.65, 0.72, 0.75, 0.72).  The corresponding values after exclusion of censored data were (0.53, 0.64, 0.68, 0.64, 0.72, 0.75, 0.71). 

We have now also found that: starting from 1989 the adjusted R-squared values for CB28:CB180 for the fitted model after substitution using LOQ/sqrt(2) were (0.54, 0.62, 0.67, 0.73, 0.86, 0.90, 0.83).  The corresponding values after exclusion of censored data were (0.52, 0.62, 0.71, 0.72, 0.86, 0.90, 0.86).

We see that the adjusted R-squared values for CB118:CB180 were higher when starting from 1989 than from 1987.  However, the values from "exclusion" or "substitution by LOQ/sqrt(2)" of censored values were very similar.

We should remain alert to the fact that all of the above related to fitting linear models so it may not be relevant if such models are unsuitable.
